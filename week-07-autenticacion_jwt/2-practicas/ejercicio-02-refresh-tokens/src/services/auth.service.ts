...
export const authService = {
  ...
  async refrescar(refreshTokenCookie: string | undefined) {
    ...
    // Rotacion: si la version del token no coincide con la actual en BD,
    // significa que ya fue usado/rotado antes o fue invalidado (logout).
    if (payload.tokenVersion !== usuario.refreshTokenVersion) {
      throw AppError.unauthorized("El refresh token ya no es valido");
    }

    // Incrementamos la version ANTES de emitir el nuevo token: esto invalida
    // de inmediato el refresh token que se acaba de usar (rotacion real).
  ...
  async logout(id: string) {
    // Invalida TODOS los refresh tokens existentes de este usuario de un solo golpe.
